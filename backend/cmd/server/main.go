...
func commandConfig() (config.Config, bool, bool) {
	...
	flag.StringVar(&cfg.StorageDriver, "storage", cfg.StorageDriver, "存储驱动：sqlite 或 mongodb")
	flag.StringVar(&cfg.DatabasePath, "database", cfg.DatabasePath, "SQLite 数据库文件路径")
	flag.StringVar(&cfg.MongoDBURI, "mongodb-uri", cfg.MongoDBURI, "MongoDB 连接地址")
	flag.StringVar(&cfg.MongoDBName, "mongodb-database", cfg.MongoDBName, "MongoDB 数据库名")
